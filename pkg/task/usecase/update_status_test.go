...
func TestUpdateStatus_Invoke(t *testing.T) {
	t.Parallel()
	...
	t.Run("正常系", func(t *testing.T) {
		...
		repo.
			...
			Save(gomock.Eq(t.Context()), &task.Task{
				Header: &task.Header{ID: "id", UserID: "userID"},
				Body:   &task.Body{Title: "title", Description: "description", Status: task.StatusWorkInProgress},
				Code:   0,
			}).
			Return(nil)
		...
		input := &UpdateStatusInput{
			Code:   0,
			Status: task.StatusWorkInProgress,
		}
		useCase := NewUpdateStatus(&appbase.MockTx{}, repo)
